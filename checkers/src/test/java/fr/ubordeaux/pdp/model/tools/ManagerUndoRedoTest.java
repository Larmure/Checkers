...
class ManagerUndoRedoTest {
    ...
    void testUndoSimpleMove() {
        ...
        assertEquals(expectedIndex, spyBoard.lastAppliedMove.getTo(), "The destination of the reverse move should be the origin of the initial move.");
    }
    ...
    void testUndoWithPromotionAndCapture() {
        ...
        move.getCapturedColors().add("BLACK"); 
        ...
        assertTrue(spyBoard.demoteBitCalled, "The demoteBit method of the board should be called if the move was a promotion.");
        assertEquals(1, spyBoard.restoredPieces.size(), "A captured piece should have been restored.");
        assertEquals(45, spyBoard.restoredPieces.get(0), "The correct position of the captured piece should be restored.");
    }

    @Test
    void testRedoMove() {
        Move move = new Move(spyBoard.squareToIndex("E5"), spyBoard.squareToIndex("F6"));
        move.getCapturedColors().add("WHITE");
        ...
        assertEquals(expectedIndex, spyBoard.lastAppliedMove.getTo(), "The destination should match the original move.");
        assertTrue(move.getCapturedColors().isEmpty(), "Captured colors should be cleared to avoid duplicates during Redo.");
    }
    ...
    private static class SpyBoard extends Board {
        ...
        @Override
        public void restorePiece(int pos, String type) {
