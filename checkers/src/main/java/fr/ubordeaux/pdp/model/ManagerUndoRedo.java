 ...
 *
 * <p>This class acts as a coordinator between the {@link Board} and the {@link History},
 * ensuring that moves are accurately recorded, reverted, and reapplied while maintaining
...
public class ManagerUndoRedo {
   ...
   * as making a new move invalidates any previously undone moves available for redo.
   *
   ...
   * a promotion occurred during the move, and restoring any captured pieces to their
   * exact previous state.
   *
   * @param isWhiteTurn a boolean indicating if it is currently white's turn before the undo
  ...
  /**
   * Reapplies the last undone move on the board and restores it to the main history.
   *
   * <p>The move's captured colors are cleared before reapplication to prevent
   * duplicating captured piece records during the board's standard applyMove phase.
   *
   * @param isWhiteTurn a boolean indicating if it is currently white's turn before the redo
  ...
  public boolean redo(boolean isWhiteTurn) {
    ...
    try {
      ...
      if (redoMove != null) {
        // Clear captured colors to avoid duplication when applyMove processes the captures again
