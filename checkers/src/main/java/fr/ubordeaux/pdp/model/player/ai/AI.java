 ...
 *
 * <p>
 * This class provides common constants, utility methods, and template methods
...
public abstract class Ai {
   ...
   *
   * <p>
   * This is the main entry point for AI decision making. It validates
   ...
   */
  public final Move getBestMove(ManagerUndoRedo undo, Board board, PlayerColor player, Evaluator evaluator) {
  ...
  protected final void validateDepth(int depth) {
    ...
    if (depth > MAX_SAFE_DEPTH) {
      throw new IllegalArgumentException("Depth too large (max " + MAX_SAFE_DEPTH + "), got: " + depth);
    }
  }
  ...
  protected final void validateTime(long timeMs) {
    if (timeMs < MIN_TIME_MS) {
      throw new IllegalArgumentException("Time too small (min " + MIN_TIME_MS + "ms), got: " + timeMs);
    }
    if (timeMs > MAX_TIME_MS) {
      throw new IllegalArgumentException("Time too large (max " + MAX_TIME_MS + "ms), got: " + timeMs);
    }
  }
   ...
   * @throws IllegalArgumentException if any parameter is null
   */
  private void validateParameters(ManagerUndoRedo undo, Board board, PlayerColor player, Evaluator evaluator) {
    if (undo == null) {
